declare module "gatsby-plugin-disqus" {
  ...
  const Disqus: React.FunctionComponent<DisqusProps>;

  interface CommentCountConfig {
    /**
     * Tells the Disqus service how to identify the current page.
     * When the Disqus embed is loaded, the identifier is used to look up
     * the correct thread.
  ...
  const CommentCount: React.FunctionComponent<CommentCountProps>;

  interface CommentEmbedProps {
    /**
     * This is used to determine the comment that gets embedded.
  ...
  const CommentEmbed: React.FunctionComponent<CommentEmbedProps>;
}
